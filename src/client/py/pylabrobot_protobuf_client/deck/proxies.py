...
class _SpatialMixin:
  ...
  def get_location_wrt(self, other, x="l", y="f", z="b"):
    resp = self._client.get_location_wrt(
      pb2.GetLocationWrtRequest(
  ...
  def get_absolute_location(self, x="l", y="f", z="b"):
    resp = self._client.get_absolute_location(
      pb2.GetAbsoluteLocationRequest(
        resource_name=self.name,
        anchor_x=str(x),
        anchor_y=str(y),
        anchor_z=str(z),
  ...
  def get_absolute_size_x(self):
    resp = self._client.get_absolute_size(pb2.GetAbsoluteSizeRequest(resource_name=self.name))
    return resp.x
  ...
  def get_highest_known_point(self):
    resp = self._client.get_highest_point(pb2.GetHighestPointRequest(resource_name=self.name))
    return resp.value
...
class ContainerProxy(_SpatialMixin, Container):
  ...
  def __init__(self, client: DeckServiceClientSync, data: pb2.ResourceData):
    Container.__init__(
      ...
      size_y=data.size_y,
      size_z=data.size_z,
      material_z_thickness=data.material_z_thickness
      if data.HasField("material_z_thickness")
      else None,
  ...
  def supports_compute_height_volume_functions(self) -> bool:
    resp = self._client.supports_compute_height_volume(pb2.ResourceByNameRequest(name=self.name))
    return resp.value
