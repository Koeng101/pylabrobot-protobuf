...
class RemoteResource(_SpatialMixin, Deck):
  ...
  def _build_tree(self, tree_node: pb2.ResourceTree, parent: Resource) -> None:
    ...
    for child_tree in tree_node.children:
      ...
      if child_data.HasField("location"):
        loc = Coordinate(child_data.location.x, child_data.location.y, child_data.location.z)
  ...
  def unassign_child_resource(self, resource):
    ...
    if not self._building:
      self._client.unassign_child(pb2.UnassignChildRequest(resource_name=resource.name))
