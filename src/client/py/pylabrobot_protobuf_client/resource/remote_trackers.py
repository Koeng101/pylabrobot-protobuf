...
class RemoteVolumeTracker:
  ...
  def add_liquid(self, volume: float) -> None:
    self._client.add_liquid(pb2.TrackerOpRequest(resource_name=self._resource_name, volume=volume))
...
class RemoteTipTracker:
  ...
  def _get_state(self) -> pb2.TipTrackerState:
    return self._client.get_tip_tracker_state(pb2.ResourceByNameRequest(name=self._resource_name))
  ...
  def remove_tip(self, commit: bool = False) -> None:
    self._client.remove_tip(pb2.TipTrackerOpRequest(tip_spot_name=self._resource_name))
    if commit:
      self.commit()

  def add_tip(self, tip: Optional[Tip] = None, origin=None, commit: bool = True) -> None:
    self._client.add_tip(pb2.TipTrackerOpRequest(tip_spot_name=self._resource_name))
    if commit:
      self.commit()
