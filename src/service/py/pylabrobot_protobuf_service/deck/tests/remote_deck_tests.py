...
class TestTipSerialization(unittest.TestCase):
  ...
  def test_plain_tip_round_trip(self):
    tip = Tip(
      has_filter=False, total_tip_length=59.9, maximal_volume=400.0, fitting_depth=8.0, name="t1"
    )
...
class TestResourceToData(unittest.TestCase):
  ...
  def test_plate_ordering(self):
    ...
    plate = Plate(
      name="p", size_x=127, size_y=85, size_z=14, ordered_items=wells, plate_type="non-skirted"
    )
