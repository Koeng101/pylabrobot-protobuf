...
def _resource_to_data(resource: Resource) -> pb2.ResourceData:
  ...
  if isinstance(resource, Well):
    data.well_bottom_type = (
      resource.bottom_type.value
      if hasattr(resource.bottom_type, "value")
      else str(resource.bottom_type)
    )
    data.cross_section_type = (
      resource.cross_section_type.value
      if hasattr(resource.cross_section_type, "value")
      else str(resource.cross_section_type)
    )
...
class ResourceServiceImpl(ResourceService):
  ...
  async def get_resource(
    self, request: pb2.ResourceByNameRequest, ctx: RequestContext
  ) -> pb2.ResourceData:
  ...
  async def get_highest_point(
    self, request: pb2.GetHighestPointRequest, ctx: RequestContext
  ) -> pb2.FloatResponse:
    resource = self._deck.get_resource(request.resource_name)
    return pb2.FloatResponse(value=resource.get_highest_known_point())

  async def batch_get_location_wrt(
    self, request: pb2.BatchGetLocationWrtRequest, ctx: RequestContext
  ) -> pb2.BatchCoordinateResponse:
    coords = []
  ...
  async def compute_volume_from_height(
    self, request: pb2.ComputeVolumeHeightRequest, ctx: RequestContext
  ) -> pb2.FloatResponse:
  ...
  async def get_volume_tracker_state(
    self, request: pb2.ResourceByNameRequest, ctx: RequestContext
